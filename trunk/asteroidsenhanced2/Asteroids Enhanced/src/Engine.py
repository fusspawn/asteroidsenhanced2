...
def load_sounds():
    ...
    sounds.append(load_sound("menu.wav"))
    return sounds

def load_font(file, size):
    return pygame.font.Font(os.path.join("data", "fonts", file), size)

def render_text(surface, text, font, pos, center=False, col=(255,255,255)):
...
def load_highscore():
    f = os.path.expanduser("~/.AstEn")
    if os.path.exists(f):
        hs = open(os.path.expanduser("~/.AstEn"), "rU").read()
        return int(hs)
    else:
        hs = open(os.path.expanduser("~/.AstEn"), "wr").write(str(0))
        return 0
    
def save_highscore(score):
    open(os.path.expanduser("~/.AstEn"), "wr").write(str(score))
...
class Sprite(pygame.sprite.Sprite):
    ...
    def draw(self, surface):
        ...
        if self.use_antialias:
            pygame.draw.aalines(surface,self.col,1,self.drawpoints,1)
        else:
