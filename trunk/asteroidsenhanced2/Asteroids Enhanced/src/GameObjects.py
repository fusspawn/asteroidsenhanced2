...
class Shot(Sprite):
    ...
    def __init__(self, pos, angle):
        ...
        Sprite.__init__(self, self.containers)
        Sprite.setCol(self, (255,0,0))
        self.pos = list(pos)
        self.points = SHOT_POINTS
        #self.points = HARVEST_BEAM_POINTS
        self.size = [2, 2]
        self.angle = angle
        self.speed = 6
...
class Asteroid(Sprite):
    ...
    def __init__(self, pos, scale=1.25):
        ...
        self.points = random.choice([ASTEROID1_POINTS, ASTEROID3_POINTS, ASTEROID4_POINTS])
        self.pos = list(pos)
        self.angle = random.randrange(360)
        self._angle = self.angle
        self.scale = scale + 0.25

        self.speed = random.choice([-0.75, -0.5, -0.25, -0.125, -0.0625, -0.625/2, 0.625/2, 0.0625, 0.125, 0.25, 0.5, 0.75])*2/self.scale/2
...
class Exhaust(Sprite):
    ...
    def draw(self, surface):
        if self.use_antialias:
            surface.fill((0, self.color, 255), (int(self.pos[0]), int(self.pos[1]), self.size[0], self.size[1]))
        else:
            surface.fill((0, self.color, 255), (int(self.pos[0]), int(self.pos[1]), self.size[0], self.size[1]))
